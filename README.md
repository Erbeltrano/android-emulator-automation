...
**"Pausa esame"** (2026-08-12): l'utente userà lo stesso PC Windows anche per esami universitari da casa con software di proctoring, e vuole poter chiedere di mettere in pausa bot/BlueStacks senza spegnere il PC (che gli serve acceso per l'esame). Nuove `pause_for_exam()`/`resume_after_exam()` in `minipc/bot_control.py`: la prima ferma bot e BlueStacks e **disabilita** (non cancella, reversibile con un comando) i task pianificati `CoCBot`/`CoCBotCostruttori`, cosi' nessun comando Telegram/dashboard arrivato per sbaglio può far ripartire nulla; la seconda li riabilita. Fermare solo i processi non garantisce l'invisibilità da un software di proctoring molto aggressivo che scansiona anche i programmi installati (non solo quelli attivi) - BlueStacks resterebbe comunque installato sul disco. Nessun trigger automatico: vanno chiamate esplicitamente su richiesta dell'utente.
