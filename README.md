...
**"Pausa esame"** (2026-08-12, l'utente farà esami universitari da casa con **Respondus LockDown Browser** sullo stesso PC Windows che fa girare il bot): nuove `pause_for_exam()`/`resume_after_exam()` in `minipc/bot_control.py`, **confermate dal vivo in entrambe le direzioni**. La prima ferma bot ed emulatore, e - scoperto dal vivo controllando cosa restava attivo anche a HD-Player chiuso - anche due componenti BlueStacks che sopravvivono in background all'insaputa dell'utente: `BlueStacksServices.exe` (il launcher/hub, si riavvia da solo ad ogni login Windows tramite una voce nel registro `HKCU...\Run`) e `BstkSVC.exe` (rilanciato ogni ora dal task pianificato `BlueStacksHelper_nxt`, creato da BlueStacks stesso, non da noi). Entrambi hanno "bluestacks" nel nome processo - un rischio concreto se un software di controllo esame scansiona i processi in esecuzione per nome. `pause_for_exam()` ferma tutti questi processi, **disabilita** (`schtasks /change .../disable`, non cancella) i tre task pianificati (`CoCBot`, `CoCBotCostruttori`, `BlueStacksHelper_nxt`) così nessun comando Telegram/dashboard arrivato per sbaglio può far ripartire nulla, e rimuove temporaneamente la voce di registro di BlueStacksServices (letta e salvata prima di rimuoverla, per poterla ripristinare parola per parola). `resume_after_exam()` riabilita tutto. **Verificato dal vivo**: dopo `pause_for_exam()`, nessun processo BlueStacks/bot/python rimasto in esecuzione, tutti e tre i task risultavano "Disabilitato"; dopo `resume_after_exam()`, tutti e tre di nuovo "Pronta" e la voce di registro ripristinata identica all'originale.

**Automatizzato per Telegram** (2026-09-01, richiesta esplicita: "durante l'anno avrò più esami quindi potrebbe ricapitare" - prima queste due funzioni erano azionabili solo a mano tramite me): nuovi comandi `/pausaesame` e `/riprendiesame` in `minipc/telegram_relay.py`, che chiamano semplicemente `pause_for_exam()`/`resume_after_exam()` già esistenti - **testato dal vivo il giro completo** (pausa → verifica task/registro sul PC Windows → ripristino → riverifica), tutto tornato identico a prima. Il servizio `telegram-relay` è stato riavviato per caricare i nuovi comandi.

**SSH volutamente escluso da questi comandi**: `pause_for_exam()` non tocca il server SSH del PC Windows, nonostante fosse già segnalato come rischio più sotto in questo stesso file ("disattiva SSH/RDP prima di ogni esame") - disattivarlo da un comando che passa *per* SSH taglierebbe fuori anche il comando stesso di ripristino, rendendo impossibile un `/riprendiesame` remoto una volta spento. Per questo pezzo (opzionale, da valutare quanto serve davvero per il software di proctoring in uso) ci sono due script locali pensati per un doppio click direttamente sul PC Windows, non azionabili da remoto: `windows/pausa_esame_ssh.bat` (ferma e disabilita il servizio `sshd`, si autoeleva a Amministratore) e `windows/riprendi_esame_ssh.bat` (lo riattiva). Vanno eseguiti nell'ordine: `/pausaesame` da Telegram prima di sedersi al PC, poi eventualmente `pausa_esame_ssh.bat` in locale se si vuole coprire anche SSH; a esame finito, `riprendi_esame_ssh.bat` in locale (se usato) e poi `/riprendiesame` da Telegram.

Limite dichiarato esplicitamente, non risolvibile lato codice: questo copre lo stato *attivo* del sistema (processi in esecuzione, avvio automatico), che è quello che LockDown Browser è documentato controllare (blocca l'avvio se rileva una VM attiva, software di condivisione schermo/accesso remoto in esecuzione, o processi in una lista nera). Non risulta invece che faccia un audit statico di *tutto ciò che è installato* sul disco - ma non essendo pubbliche le tecniche esatte di rilevamento (e potendo variare per versione/configurazione dell'ateneo), non c'è garanzia assoluta senza disinstallare BlueStacks del tutto per quel giorno (opzione più invasiva, valutata ma non implementata: rischia di richiedere di ricalibrare l'ambiente al reinstallo).
